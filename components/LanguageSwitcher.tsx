  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="sm" className="flex items-center gap-2 mx-4">
          {language === "nl" ? <NLFlag /> : <GBFlag />}
          <span>{language === "nl" ? "Nederlands" : "English"}</span>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end">
        <DropdownMenuItem onClick={() => handleLanguageChange("nl")} className="flex items-center gap-2">
          <NLFlag />
          <span className={language === "nl" ? "font-bold" : ""}>Nederlands</span>
        </DropdownMenuItem>
